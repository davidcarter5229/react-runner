...
mkdir -p ${LOG_PATH}
status_log="${LOG_PATH}/serving_infer_python.log"
...
function func_serving(){
    ...
    trans_log="${LOG_PATH}/python_trans_model_${model_name}.log"
    ...
    for use_gpu in ${web_use_gpu_list[*]}; do
        if [ ${use_gpu} = "null" ]; then
            _save_log_path="${LOG_PATH}/python_server_cpu.log"
            web_service_cmd="${python} ${web_service_py} ${web_use_gpu_key}='' > ${_save_log_path} 2>&1 "
            eval $web_service_cmd
            last_status=${PIPESTATUS[0]}
            status_check $last_status "${web_service_cmd}" "${status_log}" "${model_name}"
            sleep 5s
            set_image_dir=$(func_set_params "${image_dir_key}" "${image_dir_value}")
            _save_infer_log_path="${LOG_PATH}/python_client_cpu.log"
            pipeline_cmd="${python} ${pipeline_py} ${set_image_dir} > ${_save_infer_log_path} 2>&1 "
            eval $pipeline_cmd
            last_status=${PIPESTATUS[0]}
            ...
            _save_log_path="${LOG_PATH}/python_server_gpu.log"
            web_service_cmd="${python} ${web_service_py} ${web_use_gpu_key}=${use_gpu} > ${_save_log_path} 2>&1 "
            eval $web_service_cmd
            last_status=${PIPESTATUS[0]}
            status_check $last_status "${web_service_cmd}" "${status_log}" "${model_name}"
            sleep 5s
            set_image_dir=$(func_set_params "${image_dir_key}" "${image_dir_value}")
            _save_infer_log_path="${LOG_PATH}/python_client_gpu.log"
            pipeline_cmd="${python} ${pipeline_py} ${set_image_dir} > ${_save_infer_log_path} 2>&1 "
            eval $pipeline_cmd
            last_status=${PIPESTATUS[0]}
            status_check $last_status "${pipeline_cmd}" "${status_log}" "${model_name}"
